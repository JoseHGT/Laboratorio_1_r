...
```{r}
student_1 <- c(50.4, 62.3,92.91,77.6,TRUE)
...
```{r}
name_students <- c("Lyly Brow", "Davis Moore", "Susan Taylor", "Mike Jones", "Tom Hill")  
name_students
...
consideerar las primera 4 columnas, porque la última corresponde a la mayoría de
edad.
...
students_projection <- function(){
  for (row in 1:nrow(result_students)) {
    for(col in 1:4){
      print(paste("En Fila ", row, "Y en Columna ", col, " Contiene ", result_students[row,col]))
